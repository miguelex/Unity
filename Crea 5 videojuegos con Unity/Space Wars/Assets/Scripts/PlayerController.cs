...
public class PlayerController : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		theBody.AddForce(direction.normalized*maxSpeed); // Normalized se usa para no pasarse de 1 (en caso de movimiento simultaneo entre los dos ejes)
		
		// Actualizar la velocidad
		
		theBody.velocity = new Vector3(Mathf.Clamp(theBody.velocity.x, -maxSpeed, maxSpeed),
									   Mathf.Clamp(theBody.velocity.y, -maxSpeed, maxSpeed),
									   Mathf.Clamp(theBody.velocity.z, -maxSpeed, maxSpeed)); // Clamp impide pasar de maxSpeed 
									   
	    // Rotacion de la nave con el raton
	    
	    if(mouseLook)
	    {
		    // Posicion del raton en el juego
